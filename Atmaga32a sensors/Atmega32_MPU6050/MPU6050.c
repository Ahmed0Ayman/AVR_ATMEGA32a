 ...
 * param : ALL_Axis_Var is a data structure that in general hold all MPU read data register
 * return : void
 */
 ...
 * param : flag  used to mark that the calibration is performed successfully
 * return : void
 */
...
PRIVATRE void MPU6050_GYRO_CALIBRATION_FUN(_Bool * flag)
{
	...
	{
		HAL_I2C_Mem_Read(&hi2c2, MPU6050_DEVICE_ADDR, MPU6050_GYRO_XOUT_H_R,
				data, 6);
...
/* first before initialize gyro we need to read the accel data to make it as start value of gyro */
	...
	{
//	MPU6050_ALL_ACCEL_ANGELS(ANGELS_VAR_STRUCT);
...
void MPU6050_ALL_ACCEL_Read(MPU6050_DATA_t *ALL_Axis_Var) {
	...
	uint8_t data[6] = { 0 };
	HAL_I2C_Mem_Read(&hi2c2, MPU6050_DEVICE_ADDR, MPU6050_ACCEL_XOUT_H_R,
			data, 6);
	ALL_Axis_Var->XA_Value=		(data[0] << 8) | data[1];
	ALL_Axis_Var->YA_Value =	(data[2] << 8)| data[3];
	ALL_Axis_Var->ZA_Value =	(data[4] << 8)| data[5];
...
void MPU6050_SET_Config(uint8_t *Config_arr, uint8_t array_size) {
	...
	for (uint8_t i = 0; i < (array_size / 2); i++) {
		HAL_I2C_Mem_Write(&hi2c2, MPU6050_DEVICE_ADDR, *(Config_arr + (i * 2)),
				(Config_arr + (i * 2) + 1), 1);
	}
 ...
 */
 static void MPU6050_Read_temp_reg(MPU6050_DATA_t *ALL_Axis_Var) {
	...
	HAL_I2C_Mem_Read(&hi2c2, MPU6050_DEVICE_ADDR, MPU6050_TEMP_OUT_H_R,
			((uint8_t*) ALL_Axis_Var + 12), 2);
