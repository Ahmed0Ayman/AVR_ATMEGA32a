 ...
 void HAL_SPI_Init(SPI_Handler_t * Handler)
{
	...
	if((Handler->INIT.CLKPrescaler) & 0x80)    /* set the selected prescaler so first check if it is required to set the SPI2X bit  */
	{
	 SPI_PREPH->SPI_SPCR |=((Handler->INIT.CLKPrescaler)&0x3u);
	 SPI_PREPH->SPI_SPSR |= SPI_SPSR_SPR2;
	}else{
	 SPI_PREPH->SPI_SPCR |=((Handler->INIT.CLKPrescaler)&0x3u);		
	}
	if(Handler->INIT.Mode & SPI_SPCR_MSTR)  /* master mode GPIO configuration */ 
	{
	...
	/* set the select attribute of the SPI peripheral */
	 SPI_PREPH->SPI_SPCR  = Handler->INIT.CLKPhase | Handler->INIT.CLKPolarity |Handler->INIT.FirstBit |Handler->INIT.INI_EN |Handler->INIT.Mode ;
	 SPI_PREPH->SPI_SPCR |= SPI_SPCR_SPE ; 	
	if(Handler->INIT.INI_EN == SPI_INT_MODE_ENABLE)   /* if SPI interrupt enable then enable the global interrupt */
		sei();
