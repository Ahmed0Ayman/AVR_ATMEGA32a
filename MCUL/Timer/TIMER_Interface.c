...
void __vector_4 (void)  __attribute__ ((signal,used));			// TIMER2_COMP VECTOR 
void __vector_5 (void)  __attribute__ ((signal,used));			// TIMER2_OVERFLOW VECTOR
void __vector_6 (void)  __attribute__ ((signal,used));			// TIMER1_COMPTURE VECTOR
void __vector_7 (void)  __attribute__ ((signal,used));			// TIMER1_COMPA VECTOR
void __vector_10 (void) __attribute__ ((signal,used));			// TIMER_COMPB VECTOR
void __vector_11 (void) __attribute__ ((signal,used));			// TIMER_OVERFELOW VECTOR 
 ...
 * RETVAL(TIM_Status_t)	: TIM status  
 */ 
  TIM_Status_t TIM_DeInit(TIMInit_t * TIMConfig )
{
 ...
 * RETVAL(TIM_Status_t)	: TIM status  
 */ 
  TIM_Status_t TIM_Stop(TIMInit_t * TIMConfig )
{
	FUN_CHECK_PARAM(TIMConfig);
...
TIM_Status_t	TIM_PWMModeInit(TIMInit_t * TIMConfig )
{
	...
	if (TIMConfig->Instance == TIM1)
	{
		...
		{
			if( TIM16Bit_MODE_COMP_PWM_FAST_8B == (TIMConfig->TIMMode&TIM16Bit_MODE_COMP_PWM_FAST_8B) )
			{
				TCCR1A  =  0x01u ;
				TCCR1B  =  0x01u<<3 ;
			}
			else if(TIM16Bit_MODE_COMP_PWM_FAST_9B == (TIMConfig->TIMMode&TIM16Bit_MODE_COMP_PWM_FAST_9B ))
			{
				TCCR1A  =  0x02u ;
				TCCR1B  =  0x01u<<3 ;
			}
			else if( TIM16Bit_MODE_COMP_PWM_FAST_10B == (TIMConfig->TIMMode&TIM16Bit_MODE_COMP_PWM_FAST_10B) )
			{
				TCCR1A  =  0x03u ;
				TCCR1B  =  0x01u<<3 ;
			}
			else if( TIM16Bit_MODE_COMP_PWM_FAST_MODE == (TIMConfig->TIMMode&TIM16Bit_MODE_COMP_PWM_FAST_MODE) )
			{
				TCCR1A  =  0x02u ;
				TCCR1B  =  0x03u<<3 ;
			}
